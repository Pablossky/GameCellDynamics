...
export const HawkDoveSpacePage: React.FC = () => {
  ...
  const updatePopulationMatrix = () => {
    ...
    for (let i = 0; i < gridSize; i++) {
      ...
      for (let j = 0; j < gridSize; j++) {
        const fitness = calculateCellFitness(i, j);
        const randomValue = Math.random();

        // Adjust this logic based on your specific replacement criteria
        const newValue = randomValue < getProbability(i, j) ? 1 : 0;
  ...
  const calculateCellFitness = (rowIndex: number, colIndex: number) => {
    const N = 4;
    const F: Payoff = {
      Hawk: rewardV,
      Dove: costC,
